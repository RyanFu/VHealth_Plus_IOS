...
- (void)getBraceletorGetAllHealthDataWithCallBack:(BLEDataAcceptCallBack)resultBlock {
    [self.bleHandringDataer ASDKGetAllHealthDataWithUpdateBlock:^(id object, int errorCode) {
        if (resultBlock) resultBlock(object, errorCode);
    }];
}
...
- (void)ASDKBLEManagerDisCoverCharacteristics:(BOOL)success Peripheral:(CBPeripheral *)peripheral service:(CBService *)service error:(NSError *)error{
    ...
    if ([VHSFitBraceletStateManager nowBLEState] == FitBLEStatebindConnected) {
        ...
        CLog(@"手环链接－－－并且已经绑定");
        // 证明 - 已经绑定过手环
        NSString *mac = [VHSCommon getShouHuanMacAddress];
...
- (void)callBackReconect:(NSString *)uuidString{
    ...
    if ([VHSCommon isNullString:uuid] || [VHSCommon isNullString:name]) {
